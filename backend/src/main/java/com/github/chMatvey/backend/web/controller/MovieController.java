...
public class MovieController {
    ...
    ResponseEntity<List<MovieResponse>> getWatched(@PathVariable String username) {
        return ok(movieService.getWatched(username));
    }

    @PutMapping("/mark-watched")
    ResponseEntity<Void> markWatched(@RequestBody WatchedRequest request) {
        movieService.markWatched(request);
